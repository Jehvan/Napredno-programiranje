...
//TODO add instance variable(s)
    ArrayList<T> logsList = new ArrayList<>();
//TODO constructor
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = logsList -> logsList.stream().filter(l -> "INFO".equals(l.getType())).collect(Collectors.toCollection(ArrayList::new));

                LogProcessor<T> secondLogProcessor = logsList -> logsList.stream().filter(l -> l.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));

                        LogProcessor<T> thirdLogProcessor = logsList -> logsList.stream().sorted(Comparator.comparing(l -> l.getTimestamp())).collect(Collectors.toCollection(ArrayList::new));

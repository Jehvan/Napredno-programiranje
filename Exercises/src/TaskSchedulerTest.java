...
class Schedulers {
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        ...
        return new TaskScheduler<>() {
            ...
            List<T> tmp;
            @Override
            public List<T> schedule(T[] tasks1) {
                tmp = Arrays.stream(tasks1).sorted(Comparator.comparing((Task s) -> s.getOrder())).collect(Collectors.toList());
                return tmp;
            }
        };
    }
    ...
    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        ...
        return tasks -> {
            return Arrays.stream(tasks).filter((Task s) -> s.getOrder() > order).collect(Collectors.toList());
        };
    }
}
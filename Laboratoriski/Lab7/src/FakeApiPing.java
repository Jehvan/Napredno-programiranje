...
public class FakeApiPing {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Callable<ApiResult> a = () -> Api.get(requestId, parameter);
            tasks.add(a);
        }
        ...
        for (Callable<ApiResult> tmp : tasks) {
            futures.add(executor.submit(tmp));
        }
        ...
        for (Future<ApiResult> a : futures) {
            try {
                results.add(a.get(timeoutMillis, TimeUnit.MILLISECONDS));
            } catch (TimeoutException e) {
                results.add(new ApiResult(futures.indexOf(a) + 1, false, "TIMEOUT"));
            }
        }

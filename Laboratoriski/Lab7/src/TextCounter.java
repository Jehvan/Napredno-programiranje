...
public class TextCounter {
    ...
    public static Callable<Counter> getTextCounter(int textId, String text) {
        //TODO
        return () -> {
            int whitespaces = text.trim().isEmpty() ? 0 : text.trim().split("\\s+").length;
            int chars = text.length();
            int lines = text.isEmpty()  ? 0 : text.split("\\R").length;
            return new Counter(textId, lines, whitespaces, chars);
        };
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Callable<Counter> a = TextCounter.getTextCounter(textId, text.toString());
            tasks.add(a);
        }

...
class University {
    ...
    public int getTotalStudentCount() {
        ...
        //TODO
        return Math.toIntExact(departments.stream()
                .flatMapToInt(d -> d.getCourses()
                        .stream()
    ...
    public Optional<Course> getHardestCourse() {
        ...
        //TODO
        return departments.stream()
                .flatMap(dept -> dept.getCourses().stream())
    ...
    public Map<Integer, List<Course>> groupByDifficulty() {
        ...
        //TODO
        return departments.stream().flatMap(c -> c.getCourses().stream()).collect(Collectors.groupingBy(Course::getDifficulty));
    }
    ...
    public Map<String, List<String>> getDepartmentToCourseNames() {
        ...
        return departments
                .stream()
                .collect(Collectors.toMap(Department::getName, d -> d.getCourses().stream().map(Course::getName).collect(Collectors.toList())));
    }
    ...
    public Optional<Department> getMostPopularDepartment() {
        //TODO
        return departments.stream()
                .max(Comparator.comparingInt(dept -> dept.getCourses()
                        .stream()
                        .mapToInt(Course::getEnrolledStudents)
                        .sum()
                ));
    ...
    public Map<String, Integer> getTotalCreditsPerDepartment() {
        //TODO
        return departments.stream()
                .collect(Collectors.toMap(Department::getName, d -> d.getCourses().stream().mapToInt(Course::getCredits).sum()));
    }
    ...
    public List<Course> getTop3HardestCourses() {
        ...
        return departments.stream()
                .flatMap(d -> d.getCourses().stream())
                .sorted(Comparator.comparingInt(Course::getDifficulty).reversed()).limit(3).collect(Collectors.toList());
    }
    ...
    public Map<String, Double> getAverageDifficultyPerDepartment() {
        //TODO
        return departments.stream()
                .collect(Collectors.toMap(Department::getName, d -> d.getCourses().stream().mapToInt(Course::getDifficulty).average().orElse(0)));
    }
    ...
    public University mergeFourSmallestDepartments() {
        ...
        String name = smallestFour.stream()
                .map(Department::getName)
                .collect(Collectors.joining(" & "));

        List<Course> courses = smallestFour.stream()
        ...
        List<Department> updated = departments.stream()
                .filter(d -> !smallestFour.contains(d))
                .collect(Collectors.toList());

        updated.add(new Department(name, courses));
